  ...
  sadLoopTimer=setInterval(()=>{
    ...
    if(sadAudio.currentTime>=79){
      ...
      setTimeout(()=>{
        ...
        sadAudio.play()
          .then(()=>fadeAudio(sadAudio,.10,1100))
          .catch(()=>{});
      },740);
    }
  },250);
...
function finalText(){
  ...
  setTimeout(()=>{
    ...
    // Piyano daima arkada kalır; yalnızca bazı vurucu cümlelerde çok hafif yükselir.
    const pianoLevels = [.085,.085,.09,.09,.095,.10,.105,.095,.115,.10,.105,.11,.075];

    function setFinalPianoLevel(level, ms=850){
      if(!sadAudio || !soundOn) return;
      const safe=Math.max(.06,Math.min(.12,level));
    ...
    function showPage(){
      ...
      setFinalPianoLevel(pianoLevels[page] ?? .09, 850);
